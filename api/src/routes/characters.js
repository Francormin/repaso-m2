...
const infoApi = async () => {
  ...
  const info = url.data.map((c) => {
    return {
      id: c.char_id,
      name: c.name,
      img: c.img,
      occupation: c.occupation ? c.occupation.map((o) => o) : null,
      nickname: c.nickname,
      status: c.status,
      birthday: c.birthday,
...
router.get('/:id', async (req, res) => {
  ...
  if (id) {
    ...
    characterid.length ? res.status(200).json(characterid) : res.status(404).send('character id not found');
  }
});
